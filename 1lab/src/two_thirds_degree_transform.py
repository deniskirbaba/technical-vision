...
def two_thirds_deg_transform(i_cur):
    return cumul_hist[i_cur] ** (2 / 3)


new_img = np.squeeze(np.array([two_thirds_deg_transform(i) for i in img]))
...
cv.imshow("2/3 degree transformation", comp)
cv.waitKey(0)
